...
<body>
	<%@ include file="/common/admin_header.jsp" %><!-- 引入头部 -->
        ...
        <form action="${ctx}/admin/score" method="post" >  
		<input type="hidden" name="act" value="save"/> 
		<input type="hidden" class="form-control" name="id" readonly="readonly" value="${idscore.scoreID}">		
	 ...
	 <select name="province" id="province" class="form-control">
		...
		<input type="radio"  name="corseid" value="${course.courseID}">${course.courseName}
		</c:forEach>
		</div>  --%>
		  <h3>请输入分数</h3>
		<input type="text"  class="form-control" name="score" value="${idscore.score}">
		 </div></div><
		  <div class="layui-form-item">
		 <div class="layui-input-block">
 ...
 */
 ...
 window.onload = function(){
        ...
        */
        ...
        xmlHttp.open("GET","<c:url value='/student'/>",true);
        //发送
        xmlHttp.send(null);
        //添加监听
            ...
            if(xmlHttp.readyState == 4 ){
                if(xmlHttp.status ==200){
                    ...
                    for(var i =0; i<proArray.length;i++){
                        ...
                        optionEle.value = pro.stuId;//给<option>的实际值赋为pid,而不是name
                        var textNode = document.createTextNode(pro.stuName);//使用省名称来创建textNode
        ...
        */
        document.getElementById("province").onchange = function(){
            ...
            */
            var xmlHttp = createXMLHttpRequest();
            //打开连接 在浏览器客户端执行，执行时已经改变
            xmlHttp.open("POST","<c:url value='/student'/>",true);
            //设置请求头
                    ...
                    if(xmlHttp.status ==200){
                        ...
                        */
                        ...
                        for(var i=0; i<cityArray.length;i++){
                            ...
                            var optionEle = document.createElement("option");
                            //给<option>元素的value属性赋值                            
                            optionEle.value =city.CourseID;//给<option>的实际值赋为pid,而不是name
                            var textNode = document.createTextNode(city.course.courseName);//使用城市名称来创建textNode
                            optionEle.appendChild(textNode);//把textNode添加到option元素中
                            //把option元素添加到select元素中
